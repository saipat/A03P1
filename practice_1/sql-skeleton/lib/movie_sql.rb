...
def more_cage_please
  MovieDatabase.execute(<<-SQL)
    ...
    join castings c on c.movie_id = m.id
    join actors a on c.actor_id = a.id
    where a.name = 'Nicolas Cage' and not c.ord = 1
    order by m.title
  SQL
end
...
def count_bad_actors
  MovieDatabase.execute(<<-SQL)
    ...
    from actors a
    left outer join castings c on c.actor_id = a.id
...
def chris_is_missed
  MovieDatabase.execute(<<-SQL)
    ...
    where c.ord = 1 and m.id  in (
      select m1.id
      from movies m1
      join castings c1 on c1.movie_id = m1.id
      join actors a1 on c1.actor_id = a1.id
      where a1.name = 'Chris Farley'
    )
  SQL
end
